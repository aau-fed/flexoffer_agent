...
import org.goflex.wp2.foa.mqtt.MqttPublisher;
...
public class MqttDeviceService implements xEmsServices {
    ...
    public MqttDeviceService() {
    }

    @Autowired
    public MqttDeviceService(UserService userService,
    ...
    public DeviceDetailData updateDeviceState(UserT user, String organizationName, DeviceDetail device,
        ...
        try {
            ...
            Double currentConsumption = deviceDetailData.getValue();// * 20 / 1000;
    ...
    @Transactional
    public void discoverMqttDevices() {

        try {
            LOGGER.info("Running Mqtt Device Discovery Routine...");
            ...
            if (userT == null) {
                ...
                UserT mqttUser = new UserT();
                mqttUser.setUserName(userName);
                mqttUser.setPassword(passwordEncoder.encode(userPass));
                mqttUser.setRole(UserRole.ROLE_PROSUMER);
                UserAddress address = new UserAddress();
                mqttUser.setUserAddress(address);
                mqttUser.setRegistrationDate(new Date());
                mqttUser.setOrganization(organization.getOrganizationId());
                mqttUser.setEmail(userName + "@please-set.email");
                userT = userService.save(mqttUser);
            }

            // remove orphan houses
            List<DeviceHierarchy> existingMqttHouses =
            ...
            existingMqttHouses.forEach(h -> {
                if (!lastestHouseList.contains(h.getHierarchyName())) {
                    ...
                    List<DeviceDetail> houseDevices =
                            deviceDetailService.getDevicesByHierarchyId(h.getHierarchyId());
                    houseDevices.forEach(
                            device -> poolDeviceDetail.remove(device.getDeviceId()));
            ...
            for (JsonNode house : houseList) {
                ...
                if (heating_relay.equals("0") || hotwater_relay.equals("0") || heating_relay.equals(hotwater_relay)) {
                    ...
                    // this might have happened due to erroneous house info in mqtt house list
                    if (devices.size() != 1) {
                        devices.forEach(
                                device -> deviceDetailService.deleteDevicesByDeviceId(device.getDeviceDetailId()));
                    }
                    // remove device model from map
                    if (devices.size() != 1) {
                        devices.forEach(
                                device -> poolDeviceDetail.remove(device.getDeviceId()));
                    }
                ...
                if (hotwater_relay.equals("0")) {
                    ...
                    addDeviceToMqttUser(user, houseId, heatPumpAlias, DeviceType.HeatPump);
                    ...
                    addDeviceToMqttUser(user, houseId, boilerAlias, DeviceType.Boiler);
                } else {
                    ...
                    if (!hotwater_relay.equals(heating_relay)) {
                        ...
                        addDeviceToMqttUser(user, houseId, combinedAlias, DeviceType.HeatPump);
                    }
                }
            }
    ...
    private void addDeviceToMqttUser(UserT user, String houseId, String alias, DeviceType deviceType) {
        try {
            ...
            LOGGER.info("adding new mqtt device '" + alias + "' for house '" + houseId + "'");
            ...
            deviceDetail.setPlugType(PlugType.MQTT);
            ...
            PoolDeviceModel poolDeviceModel = new PoolDeviceModel(organizationId, deviceId, deviceType.toString(),
                    null, deviceType == DeviceType.Boiler ? 30 : 20,
                     deviceType == DeviceType.Boiler ? 40 : 25, -1.0, null, 0,
                    false, null, false, -1,
                     -1, -1, null, null);
            this.poolDeviceDetail.get(org.getOrganizationName()).put(deviceDetail.getDeviceId(), poolDeviceModel);
