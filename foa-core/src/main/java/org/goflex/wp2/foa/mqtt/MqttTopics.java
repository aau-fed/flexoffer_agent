...
public class MqttTopics {
    ...
    private List<String> fetchHouses() {
        ...
        try {
            ...
            //List<String> selectedHouses = houses.subList(0, Math.min(85, houses.size()));
            List<String> badHouses = new ArrayList<>(Arrays.asList());
            List<String> selectedHouses = houses.stream().filter(h -> !badHouses.contains(h)).collect(Collectors.toList());
            return  selectedHouses;

        } catch (HttpClientErrorException e) {
            LOGGER.error("Error retrieving house list from mqtt cloud: " + e.getMessage());
    ...
    public List<String> getHouses() {
        ...
        if (houseListUrl == null || houseListUrl.equals(""))
        {
            return null;
        }
